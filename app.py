...
def predict():
    text1 = request.form['1'] ## Fetching each input feild value one by one
    ...
    row_df = pd.DataFrame([pd.Series([text1,text2,text3,text4,text5,text6,text7,text8])])    ## creating a data frame using all the values
    print(row_df)
    prediction = model.predict_proba(row_df)  ## predicting tha output
    output = '{0:.{1}f}'.format(prediction[0][1] , 2)   ## fromatting output
